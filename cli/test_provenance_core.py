...
class ProvenanceCoreTests(unittest.TestCase):
    def test_google_manifest_verifies_google_provenance(self):
        meta = {
        ...
        self.assertEqual(result["resolvedVia"], "Google SynthID")
        self.assertTrue(any(s["id"] == "google_synthid" and s["status"] == "verified" for s in result["signals"]))
    ...
    def test_new_provider_profile_enables_c2pa_match_without_core_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            c2pa_results = verify_c2pa_manifest(meta, profiles)
        ...
        self.assertTrue(any(r["provider"] == "Acme Audio" and r["verified"] for r in official_results))
    ...
    def test_non_wav_upload_still_runs_provenance_checks(self):
        ...
        self.assertEqual(result["provenance"]["claimLevel"], "verified")
        self.assertEqual(result["provenance"]["provider"], "Google")
        self.assertTrue(any("PCM WAV" in item for item in result["provenance"]["limitations"]))
