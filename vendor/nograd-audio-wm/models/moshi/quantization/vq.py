...
class ResidualVectorQuantizer(BaseQuantizer):
    ...
    def encode_decode(self, x: torch.Tensor) -> list[torch.Tensor]:
        """Encode and decode the given input tensor. Keep gradients with straight-through estimator."""
        # encode.
        n_q = self.n_q
        if x.shape[-1] == 0:
            return torch.empty((x.shape[0], n_q, 0), device=x.device, dtype=torch.int64)
        ...
        x = self.input_proj(x)
        codes, all_pre_quant = self.vq.encode_all(x, n_q=n_q)
        # decode.
        quantized, all_post_quant = self.vq.decode_all(codes)
        quantized = (quantized - x).detach() + x
        quantized = self.output_proj(quantized)
        return codes, quantized, all_pre_quant, all_post_quant
...
class SplitResidualVectorQuantizer(BaseQuantizer):
    ...
    def encode_decode(self, x: torch.Tensor) -> torch.Tensor:
        """Encode and decode the given input tensor. Keep gradients with straight-through estimator."""
        codes, quantized, all_pre_quant, all_post_quant = self.rvq_first.encode_decode(x)
        if self.n_q > self.n_q_semantic:
            outputs_acoustic = self.rvq_rest.encode_decode(x)
            codes_ac, quantized_ac, all_pre_quant_ac, all_post_quant_ac = outputs_acoustic
            codes = torch.cat([codes, codes_ac], dim=0) # nq_sem+nq_acous b t
            quantized += quantized_ac
            all_pre_quant = torch.cat([all_pre_quant, all_pre_quant_ac], dim=0) # nq_sem+nq_acous b d t
            all_post_quant = torch.cat([all_post_quant, all_post_quant_ac], dim=0)
        return codes, quantized, all_pre_quant, all_post_quant
