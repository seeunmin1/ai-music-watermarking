...
class LMGen(StreamingModule[_LMGenState]):
    ...
    def _get_inclusive_stream_context(
        self,
        state: _LMGenState,
        stream_idx: int,
        ngram_size: int,
        ...
        """Return the last `ngram_size` tokens for one stream, including the current cache position."""
        batch_size = state.cache.shape[0]
        if ngram_size <= 0:
            return torch.zeros(batch_size, 0, device=self.lm_model.device, dtype=torch.long)

        ctx = torch.zeros(batch_size, ngram_size, device=self.lm_model.device, dtype=torch.long)
        available = min(int(ngram_size), max(0, int(available_steps)))
        if available <= 0:
            return ctx

        cache_size = state.cache.shape[2]
        position = state.offset % cache_size
        positions = torch.tensor(
            [(position - available + 1 + idx) % cache_size for idx in range(available)],
            device=self.lm_model.device,
            dtype=torch.long,
        )
        ctx[:, -available:] = state.cache[:, stream_idx, positions].to(torch.long)
        return ctx
    ...
    def step(self, input_tokens: torch.Tensor, force_epad: bool = False) -> torch.Tensor | None:
        ...
        if self.wm_stream(0):
            ...
            ngrams_ = self._get_inclusive_stream_context(
                state,
                0,
                int(self.wm_ngram),
                state.offset + 1,
            ...
            for cb_index in range(lm_model.dep_q):  # codebook index.
                ...
                if self.wm_stream(1+cb_index):  # skip text stream.
                    ...
                    if wm_window_hash is None:
                        ...
                        if int(self.wm_ngram) <= 0:
                        ...
                        elif self.wm_aux_params and self.wm_aux_params.get("multichannel", False):
                            # Multichannel hash: use the last token from the first N audio channels.
                            CT = state.cache.shape[2]
                            position = state.offset % CT
                            audio_channel_idx = lm_model.audio_offset
                            n = int(self.wm_ngram)
                            available_steps = max(0, state.offset - self.max_delay + 1)
                            if available_steps <= 0:
                                ngrams = torch.zeros(B, n, device=lm_model.device, dtype=torch.long)
                            else:
                                ctx = state.cache[:, audio_channel_idx : audio_channel_idx + n, position].to(torch.long)
                                L = ctx.shape[1]
                                if L < n:
                                    pad_len = n - L
                            ...
                            # Single-stream hash follows unified_wm_main.py: the last N tokens
                            # from the first audio stream, including the current cache position.
                            ngrams = self._get_inclusive_stream_context(
                                state,
                                lm_model.audio_offset,
                                int(self.wm_ngram),
                                state.offset - self.max_delay + 1,
                            )
