...
class StreamingAdd(StreamingModule[_StreamingAddState]):
    """A streaming module that adds two tensors along their last dimension.
    
    When in streaming mode, it handles partial overlaps between the tensors,
    storing any leftover parts in the state for the next forward pass.
    """
...
class RawStreamingConv1d(nn.Conv1d, StreamingModule[_StreamingConvState]):
    """A streaming 1D convolution layer that processes input in chunks.
    
    This layer extends PyTorch's Conv1d to support streaming operation, where input
    can be processed in chunks while maintaining the same output as if the entire
    sequence was processed at once. The layer handles the overlap between chunks
    using an internal state.
    
    Note:
        - Padding should be handled outside this layer (padding=0)
...
class RawStreamingConvTranspose1d(
    ...
    """A streaming 1D transposed convolution layer that processes input in chunks.
    
    This layer extends PyTorch's ConvTranspose1d to support streaming operation,
    handling partial outputs and maintaining state between forward passes to ensure
    consistency with non-streaming operation.
    
    Note:
        - Padding should be handled outside this layer (padding=0)
        - No dilation support (dilation=1)
        - Stride must be less than or equal to kernel_size
        - No output padding support (output_padding=0)
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.padding[0] == 0, "Padding should be handled outside."
