...
class CosyVoiceModel:
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid, wm_args=None):
        ...
        with self.llm_context, torch.cuda.amp.autocast(self.fp16 is True and hasattr(self.llm, 'vllm') is False):
            ...
            for i in token_generator:
                if False: # i in self.silent_tokens:
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
            ...
            prompt_speech_feat=torch.zeros(1, 0, 80), source_speech_token=torch.zeros(1, 0, dtype=torch.int32), stream=False, speed=1.0, return_token_sequence=False, wm_args=None, **kwargs):
        ...
        if source_speech_token.shape[1] == 0:
            p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid, wm_args))
        else:
        ...
        if stream is True:
            ...
            while True:
                ...
                if len(self.tts_speech_token_dict[this_uuid]) >= token_hop_len + self.token_overlap_len:
                    ...
                    if return_token_sequence:
                        out['ar_tokens'] = this_tts_speech_token.cpu()
                    yield out
                    with self.lock:
            ...
            this_tts_speech = self.token2wav(token=this_tts_speech_token,
                                             ...
                                             uuid=this_uuid,
                                             finalize=True)
            out = {'tts_speech': this_tts_speech.cpu(), 'ar_tokens': this_tts_speech_token.cpu()}
            yield out
        else:
            ...
            this_tts_speech = self.token2wav(token=this_tts_speech_token,
                                             ...
                                             finalize=True,
                                             speed=speed)
            yield {'tts_speech': this_tts_speech.cpu(), 'ar_tokens': this_tts_speech_token.cpu()}
        with self.lock:
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        with self.lock:
            ...
            self.hift_cache_dict[this_uuid] = None
        if source_speech_token.shape[1] == 0:
            p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid, kwargs.get("wm_args", None)))
        else:
            p = threading.Thread(target=self.vc_job, args=(source_speech_token, this_uuid))
        p.start()
        if stream is True:
