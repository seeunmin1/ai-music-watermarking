...
class CosyVoice:
    ...
    def inference_sft(self, tts_text, spk_id, stream=False, speed=1.0, text_frontend=True, return_token_sequence=False, wm_args=None):
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            model_input = self.frontend.frontend_sft(i, spk_id)
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed, return_token_sequence=return_token_sequence, wm_args=wm_args):
