...
from models.CosyVoice.cosyvoice.llm.maryland_sample import maryland_sample
...
class TransformerLM(torch.nn.Module):
    ...
    def sampling_ids(
        ...
        # Watermarking logic
        if wm_args is not None and wm_args.get('wm_method', 'none') == 'maryland':
            # Compute window_hash for current context
            wm_ngram = wm_args.get('wm_ngram', 0)
            wm_seed = wm_args.get('wm_seed', 0)
            clustering_map = wm_args.get('clustering_map', None)
            gamma = wm_args.get('wm_gamma', 0.25)
            delta = wm_args.get('wm_delta', 2.0)
            temp = wm_args.get('temp', 1.0)
            top_k = wm_args.get('top_k', 0)
            top_p = wm_args.get('top_p', 0.0)
            use_sampling = wm_args.get('use_sampling', False)

            if wm_ngram > 0:
                ctx = decoded_tokens[-wm_ngram:] if len(decoded_tokens) >= wm_ngram else [0] * (wm_ngram - len(decoded_tokens)) + decoded_tokens
                window_hash_val = sum(ctx) + wm_seed
            else:
                window_hash_val = wm_seed
                
            window_hash = torch.tensor([window_hash_val], dtype=torch.long, device=weighted_scores.device)
            top_ids = maryland_sample(
                weighted_scores.unsqueeze(0), window_hash,
                use_sampling=use_sampling, temp=temp, top_k=top_k, top_p=top_p,
                gamma=gamma, delta=delta, clustering_map=clustering_map,
    ...
    def inference(
        ...
        for i in range(max_len):
            ...
            top_ids = self.sampling_ids(logp.squeeze(dim=0), out_tokens, sampling, ignore_eos=True if i < min_len else False, wm_args=wm_args)
...
class Qwen2LM(TransformerLM):
    ...
    def inference(
        ...
        # 5. step by step decode
        for token in self.inference_wrapper(lm_input, sampling, min_len, max_len, uuid, wm_args=wm_args):
            yield token

    @torch.inference_mode()
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid, wm_args=None):
