...
class PreLookaheadLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, context: torch.Tensor = torch.zeros(0, 0, 0)) -> torch.Tensor:
        ...
        # Ensure input is long enough for conv1
        min_len1 = self.conv1.kernel_size[0]
        if outputs.size(2) < min_len1:
            pad_amt = min_len1 - outputs.size(2)
            outputs = F.pad(outputs, (0, pad_amt), mode='constant', value=0.0)
        outputs = F.leaky_relu(self.conv1(outputs))

        # Ensure input is long enough for conv2
        min_len2 = self.conv2.kernel_size[0]
        if outputs.size(2) < min_len2:
            pad_amt = min_len2 - outputs.size(2)
            outputs = F.pad(outputs, (0, pad_amt), mode='constant', value=0.0)
        ...
        # residual connection
        outputs = outputs[:, :inputs.size(1), :] + inputs
        return outputs
